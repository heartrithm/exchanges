    ...
    def test_errors(self):
        ...
        with requests_mock.mock() as m:
            with self.assertRaises(ExchangeApiException):
                m.get("https://api.bitfinex.com/v1/error", text='{"message": "Invalid Request"}', status_code=400)
                c.brequest(1, "error")
            ...
            with self.assertRaises(BitfinexNonceException):
                m.get("https://api.bitfinex.com/v1/nonce", text='{"message":"Nonce is too small."}', status_code=400)
                c.brequest(1, "nonce")

            with self.assertRaises(BitfinexNonceException):
                m.get("https://api-pub.bitfinex.com/v2/nonce", text='["error",10114,"nonce: small"]', status_code=500)
                c.brequest(2, "nonce")

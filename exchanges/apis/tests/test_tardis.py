    ...
    def test_get_ftx_futures_data_from_tardis(self):
        with requests_mock.mock() as m:
            ...
            result = self.client.brequest(
                1, "data-feeds/ftx", params={"symbols": ["BTC-PERP"], "filters": '[%7B"channel":"instrument"%7D]'}
            )
            self.assertEqual(result[self.client.RESULT_KEY]["openInterest"], 11206.9761)
            self.assertEqual(result[self.client.RESULT_KEY]["underlying"], "BTC")
